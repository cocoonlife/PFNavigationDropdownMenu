                     ...
                     delegate:(id)delegate
{
    ...
    if (self) {
        ...
        self.tableView.selectRowAtIndexPathHandler = ^(NSUInteger indexPath){
            ...
            [strongSelf setMenuTitleText:strongSelf.items[indexPath]];
            [strongSelf hideMenu];
            ...
            if (strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(dropdownMenuDidSelectItemAtIndex:)]) {
                [strongSelf.delegate dropdownMenuDidSelectItemAtIndex:indexPath];
            }
        };
    }
    return self;
...
- (void)showMenu
{
    ...
    [UIView animateWithDuration:self.configuration.animationDuration
                          delay:0
         usingSpringWithDamping:self.configuration.animationSpringDamping
          initialSpringVelocity:self.configuration.animationSpringVelocity
                        options:0
                     animations:^{
                         self.tableView.frame = CGRectMake(self.tableView.frame.origin.x,
                                                           -300,
...
- (void)hideMenu
{
    ...
    if (self.configuration.bounceOnHide) {
        [UIView animateWithDuration:self.configuration.animationDuration
                              delay:0
             usingSpringWithDamping:self.configuration.animationSpringDamping
              initialSpringVelocity:self.configuration.animationSpringVelocity
...
-(void)setAnimationSpringDamping:(CGFloat)animationSpringDamping{
    self.configuration.animationSpringDamping = animationSpringDamping;
}

-(void)setAnimationSpringVelocity:(CGFloat)animationSpringVelocity{
    self.configuration.animationSpringVelocity = animationSpringVelocity;
}

-(void)setBounceOnHide:(BOOL)bounceOnHide{
    self.configuration.bounceOnHide = bounceOnHide;
}
